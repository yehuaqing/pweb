...
from pweb.utils import safestr, iters, iterbetter
from pweb.utils.encoding import safeunicode
from pweb.utils.datastructs import sqlrow, storage, threadeddict, attrdict
from pweb import conf

try: 
    from pweb.http import debug
except:
    import sys
    debug = sys.stderr
...
class Transaction:
    ...
    def __init__(self, context):
        ...
        self.transaction_count = transaction_count = len(context.transactions)

        class transaction_engine:
            """Transaction Engine used in top level transactions."""
            def do_transact(self):
                context.commit(unload=False)
        ...
        class subtransaction_engine:
            ...
            def query(self, q):
                db_cursor = context.db.cursor()
                context.db_execute(db_cursor, SQLQuery(q % transaction_count))

            def do_transact(self):
                self.query('SAVEPOINT pweb_sp_%s')

            def do_commit(self):
                self.query('RELEASE SAVEPOINT pweb_sp_%s')

            def do_rollback(self):
                self.query('ROLLBACK TO SAVEPOINT pweb_sp_%s')
        ...
        if self.transaction_count:
            ...
            if self.context.get('ignore_nested_transactions'):
        ...
        self.context.transactions.append(self)
    ...
    def commit(self):
        if len(self.context.transactions) > self.transaction_count:
            self.engine.do_commit()
            self.context.transactions = self.context.transactions[:self.transaction_count]
...
class DB:
    ...
    def __init__(self, db_module, keywords):
        """Creates a database.
        """
        if db_module.__name__ != 'pyodbc':
            keywords.pop('driver', None)

        self.db_module = db_module
        self.keywords = keywords

        self._context = threadeddict()
        # flag to enable/disable printing queries
        self.printing = any([self.keywords.pop('printing', False), conf.settings.DEBUG_SQL])
    ...
    def _load_context(self, context):
        context.dbq_count = 0
        context.transactions = [] # stack of transactions

        if self.has_pooling:
            context.db = self._connect_with_pooling(self.keywords)
        else:
            context.db = self._connect(self.keywords)
        context.db_execute = self._db_execute

        if not hasattr(context.db, 'commit'):
            context.db.commit = lambda: None

        if not hasattr(context.db, 'rollback'):
            context.db.rollback = lambda: None

        def commit(unload=True):
            # do db commit and release the connection if pooling is enabled.
            context.db.commit()
            if unload and self.has_pooling:
                self._unload_context(self._context)
    ...
    def _unload_context(self, context):
        del context.db
    ...
    def _db_execute(self, cur, sql_query):
        ...
        try:
            ...
            if self.context.transactions:
                self.context.transactions[-1].rollback()
            else:
                self.context.rollback()
            raise

        if self.printing:
            print >> debug, '%s (%s): %s' % (round(b-a, 2), self.context.dbq_count, str(sql_query))
        return out
    ...
    def query(self, sql_query, vars=None, processed=None, _test=False):
        """
        ...
        if db_cursor.description:
            ...
            def iterwrapper():
                ...
                while row:
                    yield sqlrow(zip(names, row))
                    row = db_cursor.fetchone()
            out = iterbetter(iterwrapper())
            out.__len__ = lambda: int(db_cursor.rowcount >= 0 and db_cursor.rowcount or 0)
            out.list = lambda: [sqlrow(zip(names, x)) \
...
class MSSQLDB(DB):
    def __init__(self, **keywords):
        try:
            import pyodbc as db
            if 'pw' in keywords:
                keywords['pwd'] = keywords.pop('pw')
            if 'password' in keywords:
                keywords['pwd'] = keywords.pop('password')

            if os.name=='nt':
                keywords['DRIVER'] = '{SQL Server}'
            else:
                keywords['DRIVER'] = '{FreeTDS}'
            keywords['server'] = keywords.pop('host')
            keywords['uid'] = keywords.pop('user')
        except:
    ...
    def _process_query(self, sql_query):
        ...
        paramstyle = getattr(self, 'paramstyle', 'qmark')
        query = sql_query.query(paramstyle)
        params = sql_query.values()
    ...
    def query(self, sql_query, vars=None, processed=None, _test=False):
        ...
        if not self.context.transactions and isinstance(out, int): 
            self.context.commit()
...
class BaseSQLClass:
    def __init__(self, dbn="default", **configs):
        self._db = None
        self._dbn = dbn
        self.default_dbn = dbn
        self.configs = configs

    def __call__(self, dbn="default"):
        self._dbn = dbn
        return self

    def _init_db(self):
        self._db, i = storage(), 0
        params = dict(dbn='ENGINE', pw='PASSWORD', user='USER', db='NAME', host='HOST', port='PORT')
        for key, cfg in self.configs.iteritems():
            if hasattr(self, key): 
                raise NameError, "Can't set database name with: %s"% key

            if len(self._db) == 0:
                self.default_dbn = key
            elif key.lower() == 'default':
                self.default_dbn = key

            _cfg = dict([(k, cfg.get(v)) for k, v in params.iteritems() if not cfg.get(v) is None])
            _cfg['printing'] = cfg.get('printing') and True or False

            self._db[key]=database(**_cfg)
            i += 1

    def has_db(self, dbn):
        if self._db is None: self._init_db()
        return dbn in self._db.keys()

    def _getdb(self):
        if not self._db:
            self._init_db()
        if not self._db.has_key(self._dbn):
            raise AttributeError, "Not Found %s in settings.DATABASE"% self._dbn 
        return self._db.get(self._dbn)

    db = property(_getdb)

    def _parser(self, sql, *args):
        if not sql: return (None, {})
        if len(args)==0: return (sql, {})

        params = {}

        c = sql.count('?')
        if c > 1 and len(args)==1 and isinstance(args[0], tuple):
            args = args[0]

        if c > 0:
            if len(args) != c:
                raise AttributeError, "Args Length Error!"

            qmarks, idx = [], 0
            tags = sql.split('?')
            for tag in tags:
                if len(qmarks) > 0:
                    qmarks.append("$_pweb_qmark_%d"% idx)
                qmarks.append(tag)
                idx += 1
            sql = "".join(qmarks)

            for arg in args:
                params['_pweb_qmark_%d'% (len(params) + 1)] = arg
        return (sql, params)

    def selectone(self, sql, *args, **kwargs):
        if self.db is None: 
            return False, []
        sql_query, vars = self._parser(sql, *args)
        _test = kwargs.pop("_test", False)
        out = self.db.query(sql_query, vars=vars, _test = _test)
        if isinstance(out, int):
            self.db.context.commit()
            return False, sqlrow(zip())
        if len(out) > 0:
            return True, out[0]
        else:
            try: 
                return True, out[0] #fix pymssql cursor.rowcount is -1 at first bug
            except: 
                pass
        return True, sqlrow(zip())
    ...
    def select(self, sql, *args, **kwargs):
        ...
        #result = iter(out.list())
        #out = iterbetter(result)
        #out.list = lambda: [v for v in result]

        return True, out

    def selecttable(self, tables, vars=None, what='*', where=None, order=None, group=None, 
               limit=None, offset=None, _test=False): 
        return self.db.select(tables, vars, what, where, order, group, limit, offset, _test)

    def insert(self, table, **kwargs):
        if self.db is None: 
            return False, 0
        return True, self.db.insert(table, **kwargs)
    ...
    def update(self, table, where=None, *args, **kwargs):
        ...
        where, vars = self._parser(where, *args)
        _test = kwargs.pop("_test", False)
        return True, self.db.update(table, where, vars=vars, _test = _test, **kwargs)

    def delete(self, table, where, *args, **kwargs):
        if self.db is None:
            return False, 0
        where, vars = self._parser(where, *args)
        _test = kwargs.pop("_test", False)
        return True, self.db.delete(table, where, vars=vars, _test = _test)
    ...
    def modify(self, sql, *args, **kwargs):
        ...
        sql_query, vars = self._parser(sql, *args)
        _test = kwargs.pop("_test", False)
        return True, self.db.query(sql_query, vars=vars, _test = _test)

    def insertmany(self, table, fields, params, **kwargs):
        if len(fields)==0:
            raise AttributeError, "multiple_insert must set fields"
        if not isinstance(fields, (tuple, list)):
            fields = [x.strip() for x in str(fields).split(",")]
        if not isinstance(params, (tuple, list)) or len(params)==0:
            raise AttributeError, "multiple_insert params must a list or tuple"
        if not isinstance(params[0], (tuple, list)):
            raise AttributeError, "multiple_insert params must a list or tuple"

        values = [dict(zip(fields, x)) for x in params if len(x)==len(fields)]
        if len(values) != len(params):
            raise ValueError, "params have some length error"

        _test = kwargs.pop("_test", False)

        return True, self.db.multiple_insert(table, values, _test = _test)

    def selectpage(self, table, fields, page, pagesize, where=None, args=(), **kwargs):
        if self.db.dbname != "mssql":
            raise AttributeError, "selectpage only support mssql"
        if isinstance(fields, (tuple, list)):
            fields = ",".join([str(x).strip for x in fields if x])
        ...
        clause_n = self.db.sql_clauses(primarykey, table, where, None, order, offset, None)
        clauses_n = [self.db.gen_clause(sql, val, vars) for sql, val in clause_n if (val is not None and not (sql.lower().strip()=='where' and val.strip()==''))]
        qout_n = SQLQuery.join(clauses_n)

        if where: where += " AND"
        else: where = ""
        where += " %s NOT IN (%s)"% (primarykey, unicode(qout_n))

        clause_m = self.db.sql_clauses(fields, table, where, None, order, pagesize, None)
        clauses_m = [self.db.gen_clause(sql, val, vars) for sql, val in clause_m if (val is not None and not (sql.lower().strip()=='where' and val.strip()==''))]
        qout_m = SQLQuery.join(clauses_m)

        kargs = args + args
        sql, vars = self._parser(unicode(qout_m), *kargs)

        out = self.db.query(sql, vars=vars, _test=kwargs.pop('_test',False))
        if isinstance(out, int):
            self.db.context.commit()
            return False, []
        return True, out
...
class SQLClass(BaseSQLClass):
    def _init_db(self):
        self.configs = conf.settings.DATABASE
        self._db, i = storage(), 0
        params = dict(dbn='ENGINE', pw='PASSWORD', user='USER', db='NAME', host='HOST', port='PORT')
        for key, cfg in self.configs.iteritems():
            if hasattr(self, key): 
            ...
            self._db[key]=database(**dict([(k, cfg.get(v)) for k, v in params.iteritems() if not cfg.get(v) is None]))
            i += 1

class SQLConn:
    def __init__(self, sqlcls=None):
        if not sqlcls:
            sqlcls = SQLClass()
        self._sqlclass = sqlcls

    def __getattr__(self, name):
        if self._sqlclass.has_db(name):
            return self._sqlclass(name)
        elif hasattr(self._sqlclass, name):
            return getattr(self._sqlclass(self._sqlclass.default_dbn), name)
        else:
            raise AttributeError, "Not Found %s in settings.DATABASE"% name 
