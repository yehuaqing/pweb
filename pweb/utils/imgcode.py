...
class picChecker(): 
    ...
    def __init__(self,chars = initChars(),size = (95, 27),fontsize = 20, \
                    begin = (5,-2),outputType = 'PNG',mode = 'RGB' , \
                    backgroundColor = (255,255,255), foregroundColor = (0,0,255), \
                    fonttype = None,length = 4,jamNum = (2,3), \
                    pointBorder = (31,30)): 
        """ 
        初始化配置 
        """ 
        ...
        if isWindowsSystem():
            self.fonttype = fonttype or 'c:/windows/fonts/arial.ttf'
        elif isOSXSystem():
            self.fonttype = fonttype or '/Library/Fonts/Arial.ttf'
        elif isLinuxSystem():
            self.fonttype = fonttype or '/usr/share/fonts/arial.ttf'
    ...
    def out(self):
        randStr, im = self.createChecker()
        o = StringIO()
        im.save(o, self.outputType)
        o.seek(0)
        return (randStr, o.read())
