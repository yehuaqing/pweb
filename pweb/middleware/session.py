...
from pweb import http, utils, conf
from pweb.utils.datastructs import threadeddict, storage
...
class Session(object):
    ...
    def _loadconfig(self):
        self._config.cookie_name        = conf.settings.SESSION_COOKIE_NAME
        self._config.cookie_domain      = conf.settings.SESSION_COOKIE_DOMAIN
        self._config.cookie_path        = conf.settings.SESSION_COOKIE_PATH
        self._config.timeout            = conf.settings.SESSION_COOKIE_TIMEOUT
        self._config.ignore_expiry      = conf.settings.SESSION_COOKIE_IGNORE_EXPIRY
        self._config.ignore_change_ip   = conf.settings.SESSION_COOKIE_IGNORE_CHANGE_IP
        self._config.secret_key         = conf.settings.SESSION_COOKIE_SECRET_KEY
        self._config.expired_message    = conf.settings.SESSION_COOKIE_EXPIRED_MESSAGE
        self._config.httponly           = conf.settings.SESSION_COOKIE_HTTPONLY
        self._config.secure             = conf.settings.SESSION_COOKIE_SECURE
    ...
    def _setcookie(self, session_id, expires='', **kw):
        ...
        http.setcookie(cookie_name, session_id, expires=expires, domain=cookie_domain, httponly=httponly, secure=secure, path=cookie_path)
...
class Store:
    ...
    def decode(self, session_data):
        ...
        try: return pickle.loads(pickled)
        except: return {}
